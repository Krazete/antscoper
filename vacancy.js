...
function step1(){
	...
	.map(function(e){return year + "-" + e;}); // Append current year to each term code.
...
function step2(t){
	...
	var i, j, bldg = Object.keys(room);
	for(i = 0; i < bldg.length; i++){
		for(j = 0; j < room[bldg[i]].length; j++){
...
function step4(v, t, l, r){
	...
	.map(function(e){return e.innerHTML.trim().replace(/&nbsp;/g, "");}) // Remove trailing and nonbreaking spaces.
	.filter(function(e){return e.match(/(Su|M|Tu|W|Th|F|Sa)+ +\d+:\d+.+\d+:\d+/);}) // Keep properly format cells.
	.map(function(e){return e.split(/[ -]+|:/).slice(1);}) // Split each cell into starting and ending hour and minute.
	.forEach(function(e){
		if(!unique.some(function(o){return o[0] == e[0] && o[1] == e[1] && o[2] == e[2] && o[3] == e[3];})){
	...
	.map(function(e){return [e[0] + e[1] / 60, e[2] + (e[3] + 10) / 60];}) // Convert to approximate decimal hours.
	.forEach(function(e){
		var before = occupied.find(function(o){return o[1] == e[0];}); // Find classes that end when e starts.
		var after = occupied.find(function(o){return e[1] == o[0];}); // Find classes that start when e ends.
		if(before && after){
			before[1] = after[1];
	...
	if(!occupied.some(function(e){return e[0] <= t.now && t.now <= e[1];})){ // Keep location if now is not in any occupied times.
...
function step6(v, c){
	...
	var rooms, bldgs = [];
	v.forEach(function(e){if(!bldgs.includes(e.bldg)){bldgs.push(e.bldg);}}); // Compile list of unique buildings.
	bldgs.sort(function(e, o){return distance(geo[e], c) - distance(geo[o], c);});
	
	var i, j, help, available = "Available Classrooms:\n";
	for(i = 0; i < bldgs.length; i++){
		rooms = v.filter(function(e){return e.bldg == bldgs[i];}) // Get all locations of building.
		.map(function(e){return e.room;}) // Get all rooms from buidling.
		.sort(); // Sort rooms.
